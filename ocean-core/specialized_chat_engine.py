...
# =============================================================================
# HYBRID CHAT ENGINE - FULL SYSTEM INTEGRATION
# =============================================================================
class HybridChatEngine(SpecializedChatEngine):
    """
    HYBRID CHAT ENGINE - Lidhet me të gjithë sistemin Clisonix
    ...
    def __init__(self):
        ...
        logger.info("🔗 Hybrid Chat Engine initialized - Full System Integration")
    
    def _init_connections(self):
        """Initialize connections to all system components"""
        try:
            from central_api_connector import CentralAPIConnector
            self.central_api = CentralAPIConnector()
            logger.info("✅ Central API Connector loaded")
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"✅ Laboratory Network loaded: {len(self.labs.labs)} labs")
        except Exception as e:
            logger.warning(f"Labs not available: {e}")
        
        try:
            from orchestrator_v5 import get_orchestrator
            self.orchestrator = get_orchestrator()
            logger.info("✅ Orchestrator v5 loaded")
        except Exception as e:
            logger.warning(f"Orchestrator not available: {e}")
    
    # ============ KITCHEN INTEGRATION ============
    async def query_kitchen(self, data: Any = None) -> Dict[str, Any]:
        """Query Protocol Kitchen pipeline"""
        try:
            if self.central_api:
                status = await self.central_api.fetch("/api/kitchen/status")
                layers = await self.central_api.fetch("/api/kitchen/layers")
                return {
            ...
            logger.warning(f"Kitchen query error: {e}")
        return {"status": "offline", "error": str(e) if 'e' in locals() else "Not connected"}
    
    # ============ EXCEL INTEGRATION ============
    async def query_excel(self, report_type: str = "system") -> Dict[str, Any]:
        """Query Excel reporting service"""
        try:
            if self.central_api:
                # Get available reports
                reports = await self.central_api.fetch("/api/reporting/status")
                return {
                    "excel_available": True,
                    "reports": reports,
                    "export_url": "/api/reporting/export-excel",
                    "source": "central_api"
                }
        except Exception as e:
            logger.warning(f"Excel query error: {e}")
        return {"excel_available": False, "error": str(e) if 'e' in locals() else "Not connected"}
    
    # ============ POSTMAN INTEGRATION ============
    async def query_postman(self) -> Dict[str, Any]:
        """Query Postman collections and sync status"""
        try:
            if self.central_api:
                sync_status = await self.central_api.fetch("/api/postman/sync-status")
                collections = await self.central_api.fetch("/api/postman/collections")
                return {
            ...
            logger.warning(f"Postman query error: {e}")
        return {"postman_connected": False, "error": str(e) if 'e' in locals() else "Not connected"}
    
    # ============ ASI TRINITY INTEGRATION ============
    async def query_asi(self, agent: str = "all") -> Dict[str, Any]:
        """Query ASI Trinity (Alba, Albi, Jona)"""
        try:
            if self.central_api:
                if agent == "all":
                    alba = await self.central_api.fetch("/api/asi/alba/status")
                    albi = await self.central_api.fetch("/api/asi/albi/status")
                    jona = await self.central_api.fetch("/api/asi/jona/status")
                    return {"alba": alba, "albi": albi, "jona": jona, "source": "central_api"}
                else:
                    return await self.central_api.fetch(f"/api/asi/{agent}/status")
        except Exception as e:
            logger.warning(f"ASI query error: {e}")
        return {"status": "offline"}
    
    # ============ LABS INTEGRATION ============
    async def query_labs(self, lab_name: str = None) -> Dict[str, Any]:
        """Query 23 laboratories"""
        try:
            if self.labs:
                if lab_name:
                    return self.labs.query_lab(lab_name)
                return {
                    "total_labs": len(self.labs.labs),
                    "labs": [lab.to_dict() for lab in self.labs.labs.values()],
                    "source": "local"
                }
            elif self.central_api:
                return await self.central_api.fetch("/api/ocean/labs/list")
        except Exception as e:
            logger.warning(f"Labs query error: {e}")
        return {"total_labs": 0, "error": str(e) if 'e' in locals() else "Not connected"}
    
    # ============ CYCLES INTEGRATION ============
    async def query_cycles(self) -> Dict[str, Any]:
        """Query cycle alignments"""
        try:
            if self.central_api:
                return await self.central_api.fetch("/api/cycles/current")
        except Exception as e:
            logger.warning(f"Cycles query error: {e}")
        return {"status": "offline"}
    ...
    async def get_full_system_status(self) -> Dict[str, Any]:
        ...
        if self.central_api:
            try:
                api_status = await self.central_api.fetch("/api/system-status")
                status["connections"]["central_api"] = {"status": "online", "data": api_status}
            except:
                status["connections"]["central_api"] = {"status": "offline"}
        
        # Check Labs
        if self.labs:
            status["connections"]["laboratories"] = {
                "status": "online",
        ...
        if self.orchestrator:
            status["connections"]["orchestrator_v5"] = {"status": "online"}
        
        return status
    
    # ============ HYBRID RESPONSE GENERATION ============
    async def generate_hybrid_response(self, query: str, use_all_sources: bool = True) -> Dict[str, Any]:
        """
        Generate response using ALL connected systems.
        This is the main entry point for hybrid chat.
        """
        # Auto-detect domain
        domain = self.detect_domain(query)
        ...
        if use_all_sources:
            # Gather data from relevant systems based on query
            query_lower = query.lower()
            
            # Kitchen-related
            if any(kw in query_lower for kw in ["kitchen", "pipeline", "layer", "process"]):
                context["kitchen"] = await self.query_kitchen()
            
            # Excel-related
            if any(kw in query_lower for kw in ["excel", "report", "export", "raport"]):
                context["excel"] = await self.query_excel()
            
            # Postman-related
            if any(kw in query_lower for kw in ["postman", "api", "collection", "sync"]):
                context["postman"] = await self.query_postman()
            
            # ASI-related
            if any(kw in query_lower for kw in ["asi", "alba", "albi", "jona", "trinity"]):
                context["asi"] = await self.query_asi()
            
            # Labs-related
            if any(kw in query_lower for kw in ["lab", "laboratory", "sensor", "iot"]):
                context["labs"] = await self.query_labs()
            
            # Cycles-related
            if any(kw in query_lower for kw in ["cycle", "alignment", "planetary"]):
                context["cycles"] = await self.query_cycles()
        
        # Generate answer using Ollama with full context
        answer = await self._generate_contextual_hybrid_answer(query, context)
        ...
        response = {
            ...
            "confidence": 0.95 if len(context) > 3 else 0.85,
        ...
        # Store in history
        user_msg = ChatMessage(role="user", content=query, domain=domain)
        assistant_msg = ChatMessage(role="assistant", content=answer, domain=domain)
        self.chat_history.append(user_msg)
        self.chat_history.append(assistant_msg)
        
        return response
    
    async def _generate_contextual_hybrid_answer(self, query: str, context: Dict[str, Any]) -> str:
        """Generate answer using Ollama with full system context"""
        if OLLAMA_AVAILABLE:
            try:
                ollama = get_ollama_engine()
                if ollama:
                    ...
                    for key, value in context.items():
                        if key not in ["query", "timestamp"]:
                            context_info.append(f"- {key}: {str(value)[:200]}")
                    
                    system = f"""You are Ocean AI, the intelligent assistant for Clisonix Cloud Platform.
You have access to the FULL Clisonix system:

AVAILABLE DATA SOURCES:
{chr(10).join(context_info) if context_info else '- General knowledge mode'}

CRITICAL RULES:
1. ALWAYS respond in the SAME LANGUAGE as the user's question
2. Use the context data when relevant to the question
3. Be specific and provide real data when available
4. You are Ocean AI by Clisonix, created by Ledjan Ahmati
5. Keep responses professional and concise

About Clisonix:
- Founder & CEO: Ledjan Ahmati
- Organization: WEB8euroweb GmbH
- Features: Industrial Intelligence, REST APIs, IoT/LoRa sensors, Protocol Kitchen, 23 Labs, ASI Trinity
"""
                    
                    response = await ollama.generate(query, system=system)
                    if response and response.content:
                        logger.info(f"🧠 Hybrid response generated with {len(context)} context sources")
                        return response.content
            except Exception as e:
                logger.warning(f"Ollama error in hybrid chat: {e}")
        
        return f"I can help you explore this topic. Context available: {list(context.keys())}"
...
def get_hybrid_chat() -> HybridChatEngine:
    """Get or create hybrid chat engine instance"""
    global _hybrid_chat
    if _hybrid_chat is None:
        _hybrid_chat = HybridChatEngine()
    return _hybrid_chat
...
async def initialize_hybrid_chat():
    """Initialize the hybrid chat engine with full system integration"""
    global _hybrid_chat
    _hybrid_chat = HybridChatEngine()
    logger.info("🔗 Hybrid Chat Engine initialized - Full System Integration")
    return _hybrid_chat
